              ...
              eg.declineFriendRequest(data.friend.id).then(async (ac_result) => {
                ...
                console.log('[INFO] If it was you, change your name in config!');
                  ...
                  fortnite.communicator.on('party:member:state:updated', async (member) => {
                    ...
                    var jsonemote = JSON.parse(member.meta.schema.FrontendEmote_j)

                    var assetemote = jsonemote.FrontendEmote.emoteItemDef.slice(`'`);

                    var assetemotechar = assetemote.substring(asset.lastIndexOf(".") + 1)

                    var EmoteProfile = assetemotechar.replace(`'`, ``);

                    if(Features.copy.everything == true) {
                      if(!Features.copy.emote == false) {
                        
                var jsonchar = JSON.parse(member.meta.schema.AthenaCosmeticLoadout_j);

                var assetchar = jsonchar.AthenaCosmeticLoadout.characterDef.slice(`'`);

                var assetcha = assetchar.AthenaCosmeticLoadout.characterDef.slice(`'`);

                var assetch = assetcha.substring(asset.lastIndexOf(".") + 1)

                var CharacterDef = assetch.replace(`'`, ``);
                
  
                if(cid == CharacterDef) {
                  fortnite.party.me.clearEmote();
                  return fortnite.party.me.setEmote(EmoteProfile);
                }
                ...
                var pickaxe = memberprofile.AthenaCosmeticLoadout.pickaxeDef.slice(`'`);

                var pickaxeasseta = pickaxe.AthenaCosmeticLoadout.pickaxeDef.slice(`'`);

                var pickaxeassetb = pickaxeasseta.AthenaCosmeticLoadout.pickaxeDef.slice(`'`);

                var pickaxeassetc = pickaxeassetb.substring(asset.lastIndexOf(".") + 1)

                var PickaxeDef = pickaxeassetc.replace(`'`, ``);

  
                var backling = memberprofile.AthenaCosmeticLoadout.backpackDef.slice(`'`);

                var backlingasseta = backling.AthenaCosmeticLoadout.backpackDef.slice(`'`);

                var backlingassetb = backlingasseta.AthenaCosmeticLoadout.backpackDef.slice(`'`);

                var backlingassetc = backlingassetb.substring(asset.lastIndexOf(".") + 1)

                var BacklingDef = backlingassetc.replace(`'`, ``);
  
                var Variants = memberprofile.AthenaCosmeticLoadout.variants.slice(`'`);
  
                fortnite.party.me.setOutfit(CharacterDef, Variants); //Sets the outfit of the member
  
                fortnite.party.me.setPickaxe(PickaxeDef, Variants)
  
                fortnite.party.me.setBackpack(BacklingDef, Variants);
